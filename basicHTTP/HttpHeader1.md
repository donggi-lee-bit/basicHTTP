...
### Location
페이지 리다이렉션
- 웹 브라우저는 300대 응답의 결과에 Location 헤더가 있으면, Location 위치로 자동 이동(리다이렉트)
- 응답에서 3xx 에서 설명
...
- Allow : GET, HEAD, PUT
- 아직 구현이 많이 되어있지 않아 이런 게 있다 정도만 알면 됨
...
### 쿠키 미사용
1. GET /welcome HTTP/1.1 로 welcome 페이지에 접근
2. POST /login HTTP 1.1 user=홍길동
3. 홍길동으로 로그인 후 welcome 페이지에 접근해도 GET /welcome HTTP/1.1로 접근하기 때문에 로그인 하기 전처럼 된다.

모든 요청과 링크에 사용자 정보를 포함하는 대안이 있지만
- 모든 요청에 사용자 정보가 포함되도록 개발 해야함
- 브라우저를 완전히 종료하고 다시 열게되면?


### Stateless
- HTTP는 무상태(Stateless) 프로토콜이다
- 클라이언트와 서버가 요청 응답을 주고 받으면 연결이 끊어진다
...
- POST /login HTTP/1.1 user=홍길동 : 웹 브라우저에서 user는 홍길동이라는 요청을 보낸다
- HTTP/1.1 200 OK Set-Cookie: user=홍길동 : 서버는 쿠키 저장소에 user=홍길동을 저장하고 응답한다
- 로그인 이후 welcome 페이지에 접근하게 되면 쿠키 저장소에서 Cookie: user=홍길동을 같이 서버로 보내게 되고 서버는 로그인 상태를 유지한 채 응답하게 된다.
...
- 쿠키 정보는 항상 서버에 전송됨
    ...
    - 서버에 전송하지 않고, 웹 블아ㅜ저 내부에 데이터를 저장하고 싶으면 웹 스토리지 (localStorage, sessionStorage) 참고
- 주의!
    - 보안에 민감한 데이터는 저장하지 않는다 (주민번호, 신용카드 번호 등등)
...
- Set-Cookie: **max-age**=3600 (3600초)
    - 0이나 음수를 지정하면 쿠키 삭제
- 세션 쿠키 : 만료 날짜를 생략하면 브라우저 종료시까지만 유지 (크롬을 완전 종료 후 다시 켜게됐을 때)
...
- 명시 : 명시한 문서 기준 도메인 + 서브 도메인 포함
    - domain=example.org를 지정해서 쿠키 생성
        - example.org는 물론이고
        - dev.example.org도 쿠키가 접근
- 생략 : 현재 문서 기준 도메인만 적용
    - example.org에서 쿠키글 생성하고 domain 지정을 생략
        - example.org에서만 쿠키 접근
        - dev.example.org는 쿠키가 접근하지 X

### 쿠키 - 경로
Path
- 예) path=/home
- 이 경로를 포함한 하위 경로 페이지만 쿠키가 접근한다
- 일반적으로 path=/루트로 지정한다
- 예)
    - path=/home 지정
    - /home -> 가능
